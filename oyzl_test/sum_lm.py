...
TASKS = ["winogrande", "hellaswag", "gsm8k_llama", "mmlu_cot_llama"]  # 已完成四类日志
...
def parse_lmeval_table_lines(text: str) -> List[Dict[str, str]]:
    ...
    for line in text.splitlines():
        ...
        # 典型：["", "gsm8k_llama", "3", "flexible_extract", "8", "exact_match", "↑", "0.7324", "±", "0.0122", ""]
        if len(parts) < 10:
            continue
        ...
        # 跳过表头
        if task.lower() in ("tasks", "task", "groups", "group") and metric.lower() == "metric":
            continue
...
def choose_metric(task: str, metric_values: Dict[str, Tuple[Optional[float], Optional[float]]]) -> Tuple[Optional[str], Optional[float], Optional[float]]:
    ...
    - mmlu_cot_llama: key 是 "filter::metric"，只取 "strict_match::exact_match"
    """
    if task == "hellaswag":
        # 只取 acc_norm（不回退 acc）
    ...
    if task == "gsm8k_llama":
        # 优先 flexible_extract 的 exact_match
    ...
    if task == "mmlu_cot_llama":
        # 只取总分：mmlu_cot_llama + strict_match + exact_match
        k = "strict_match::exact_match"
        if metric_values.get(k, (None, None))[0] is not None:
            v, s = metric_values[k]
            return k, v, s
        return None, None, None
...
def iter_log_files(log_dirs: List[Path]) -> List[Path]:
    files: List[Path] = []
    for d in log_dirs:
...
def main():
    ...
    ap.add_argument(
        ...
        type=str,
        nargs="+",
        default=["lm_eval_logs_3tasks", "lm_eval_logs_mmlu_cot"],
        help="一个或多个日志目录（空格分隔），例如：--log_dirs lm_eval_logs_3tasks lm_eval_logs_mmlu_cot",
    )
    ...
    existing = [p for p in log_dirs if p.is_dir()]
    if not existing:
        raise SystemExit(f"[ERROR] none of log_dirs exist: {', '.join(str(p) for p in log_dirs)}")

    # model -> task -> metric_key -> (value, stderr)
    store: Dict[str, Dict[str, Dict[str, Tuple[Optional[float], Optional[float]]]]] = {}

    log_files = iter_log_files(existing)
    for lf in log_files:
        ...
        for r in rows:
            ...
            # gsm8k / mmlu_cot 需要把 filter 编进 key，避免覆盖
            if task in ("gsm8k_llama", "mmlu_cot_llama"):
    ...
    for model in sorted(store.keys(), key=lambda x: (x != "BASE", x)):
        ...
        for task in TASKS:
            ...
            elif task == "mmlu_cot_llama":
                row["mmlu_cot_metric"] = m or ""
                row["mmlu_cot"] = f"{v:.4f}" if v is not None else ""
                row["mmlu_cot_stderr"] = f"{s:.4f}" if s is not None else ""

        if len(vals_for_avg) == 4:
            row["avg_4tasks"] = f"{sum(vals_for_avg) / 4:.4f}"
    ...
    with out_csv.open("w", newline="", encoding="utf-8") as f:
        ...
        w.writerow([
            ...
            "mmlu_cot_metric", "mmlu_cot", "mmlu_cot_stderr",
            "avg_4tasks",
        ])
        for r in summary_rows:
            w.writerow([
                ...
                r["mmlu_cot_metric"], r["mmlu_cot"], r["mmlu_cot_stderr"],
                r["avg_4tasks"],
            ])
